...
async function getInstanceBrowser(user) {
  const {
    executableBrowserPath: executablePath = '',
    browserProfilePath: userDataDir = '',
  } = user;
...
async function detectionLogin(page) {
  ...
  async function checkByExistLoginButton() {
    const isExistLoginButton = document.querySelector(
      'paper-button.style-scope.ytd-button-renderer.style-suggestive[aria-label="Войти"]'
    );
