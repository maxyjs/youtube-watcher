async function addVideoToPlaylist(instanceBrowser, id, addPlaylistMark) {
  ...
  const result = await instanceBrowser.page.evaluate(async (addPlaylistMark, id, videoUrl) => {

    try {
      const saveVideoButton = document.querySelector('#top-level-buttons > ytd-button-renderer:nth-child(4) > a')
    ...
    async function getTargetPaperCheckbox(playlistsElem, paperCheckboxes, addPlaylistMark) {
  ...
  async function detectViewedVideo(page) {
    ...
    result = await page.evaluate(async () => {
        ...
        const buttonsAll = [...document.querySelectorAll('#button.style-scope.yt-icon-button')]
