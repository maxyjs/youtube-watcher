...
async function findNewResultsForUser(user, cacheAddedVideoToPlaylist) {
  ...
  console.log('\x1b[36m%s\x1b[0m', "Отфильтрованные по уже просмотренным пользователем: ", filteredByViewed.length)

  const filteredByAdded = filteredByViewed.filter(result => {
    const isAdded = cacheAddedVideoToPlaylist.checkCacheIncludeVideoId(result.video.id)
    return !isAdded
  })

  console.log('\x1b[36m%s\x1b[0m', "Отфильрованные по уже добавленным в плейлист: ", filteredByAdded.length)
