...
async function getAllResults(listQueries) {
  console.log('\x1b[36m%s\x1b[0m', "Количество поисковых запросов: ", listQueries.length);
...
async function handleResultsForAllTerms(resultsForAllTerms) {
  ...
  let combinedResultsForAllTerms = combineResultsForAllTerms(resultsForAllTerms)
  console.log('\x1b[36m%s\x1b[0m', "Общее количество результатов: ", combinedResultsForAllTerms.length);

  let allResultsWithoutDuplicates = deleteDuplicates(combinedResultsForAllTerms)
  console.log('\x1b[36m%s\x1b[0m', "Уникальные результаты: ", allResultsWithoutDuplicates.length);

  let filteredByViews = filterByViews(allResultsWithoutDuplicates)
  console.log('\x1b[36m%s\x1b[0m', "Отфильтрованные по количеству просмотров: ", filteredByViews.length);
  ...
  console.log('\x1b[36m%s\x1b[0m', "Отфильтрованные по рейтингу: ", filteredAllResults.length);
...
function combineResultsForAllTerms(resultsForAllTerms) {
  ...
  let combinedResultsForAllTerms = resultsForAllTerms.reduce((acc, resultsForTerm, index) => {
...
async function addDataVideoInfoToAllResults(allResults) {
  ...
  for await (const result of allResults) {
    ...
    try {
      ...
      console.error('\x1b[36m%s\x1b[0m', "[addDataVideoInfoToAllResults] \n ", err);
    }
...
function filterByRating(allResults) {
  ...
  const filtered = allResults.filter(result => {
    ...
    const isHR = result.video.videoInfo.rating && result.video.videoInfo.rating > minRating
    return isHR
  })
