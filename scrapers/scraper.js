...
function parseHtml(html) {
  ...
  try {
    ...
    try {
      ...
      dataText2 = dataText.substring(0, dataText.indexOf('window["ytInitialPlayerResponse"]') - 6)
  ...
  const sectionLists = data.contents.twoColumnSearchResultsRenderer.primaryContents.sectionListRenderer.contents;

  sectionLists.forEach(sectionList => {
    if (sectionList.itemSectionRenderer && sectionList.itemSectionRenderer.contents) {
